...
impl Game {
    ...
    fn pre_render(&mut self) {
        ...
        for num in 0..POP_NUM as usize {
            ...
            self.canvas.filled_circle(x, y, 7, Color::RGB(0, 150, 0));
        }
        self.canvas.present();
    }
    ...
    fn update_render(&mut self) {
        ...
        match self.status {
            GameStatus::Running => {
                ...
                time_cost = start_time.duration_since(self.time_stamp)
                    .unwrap()
                    .as_secs_f64();
                for num in 0..POP_NUM as usize {
                    entity_move2(&mut self.position_vector_list, &self.destination_vector_list, time_cost);
                    let (x, y) = self.position_vector_list[num].get_nums();
                    self.canvas.filled_circle(x, y, 5, Color::RGB(0, 150, 0)).unwrap();
                }
            },
            GameStatus::Paused => {
                start_time = SystemTime::now();
